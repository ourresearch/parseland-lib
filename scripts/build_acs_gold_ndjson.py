"""Convert American Chemical Society rows of merged-FINAL.csv into NDJSON.

Mirrors build_taylor_gold_ndjson.py. Selects rows whose DOI starts with the
ACS registrant prefix. The ACS parser's own host check (fires on
acs.org canonicals) handles stragglers whose Link resolves elsewhere.

Prefix included (verified host_organization_name = "American Chemical Society"
on the live OpenAlex API, 2026-06-02, incl. ACS Symposium Series books and C&EN):
    10.1021 — American Chemical Society primary registrant

Source:    parseland-eval/eval/data/merged-FINAL.csv  (read-only)
Sink:      parseland-lib/tests/fixtures/acs-gold.ndjson
...
OUTPUT_PATH = Path(
    "/Users/shubh-trips/Documents/OpenAlex/parseland-lib/tests/fixtures/acs-gold.ndjson"
)
...
def main() -> int:
    ...
    skipped_no_number = 0
    skipped_not_match = 0
    per_prefix: dict[str, int] = {p: 0 for p in ACS_DOI_PREFIXES}
    with CSV_PATH.open(encoding="utf-8") as f:
        reader = csv.DictReader(f)
        for row in reader:
            doi = (row.get("DOI") or "").strip().lower()
            matched = next(
                (p for p in ACS_DOI_PREFIXES if doi.startswith(p)), None
            )
            if matched is None:
                skipped_not_match += 1
                continue
    ...
    print(
        f"wrote {len(rows)} American Chemical Society rows to {OUTPUT_PATH} "
        f"({per_prefix_str}; skipped: {skipped_no_number} missing-No, "
        f"{skipped_not_match} non-ACS)"
    )
    return 0
