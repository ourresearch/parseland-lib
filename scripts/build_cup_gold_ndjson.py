"""Convert Cambridge University Press rows of merged-FINAL.csv into NDJSON.

Mirrors build_taylor_gold_ndjson.py. Selects rows whose DOI starts with the
Cambridge UP registrant prefix. The CUP parser's own host check (fires on
cambridge.org canonicals) handles stragglers whose Link resolves elsewhere.

Prefix included (verified host_organization_name = "Cambridge University Press"
on the live OpenAlex API, 2026-06-02, incl. Cambridge eBooks / cbo* DOIs):
    10.1017 — Cambridge University Press primary registrant

Source:    parseland-eval/eval/data/merged-FINAL.csv  (read-only)
Sink:      parseland-lib/tests/fixtures/cup-gold.ndjson
...
OUTPUT_PATH = Path(
    "/Users/shubh-trips/Documents/OpenAlex/parseland-lib/tests/fixtures/cup-gold.ndjson"
)

CUP_DOI_PREFIXES = ("10.1017",)
...
def main() -> int:
    ...
    skipped_no_number = 0
    skipped_not_match = 0
    per_prefix: dict[str, int] = {p: 0 for p in CUP_DOI_PREFIXES}
    with CSV_PATH.open(encoding="utf-8") as f:
        reader = csv.DictReader(f)
        for row in reader:
            doi = (row.get("DOI") or "").strip().lower()
            matched = next(
                (p for p in CUP_DOI_PREFIXES if doi.startswith(p)), None
            )
            if matched is None:
                skipped_not_match += 1
                continue
    ...
    print(
        f"wrote {len(rows)} Cambridge University Press rows to {OUTPUT_PATH} "
        f"({per_prefix_str}; skipped: {skipped_no_number} missing-No, "
        f"{skipped_not_match} non-CUP)"
    )
    return 0
