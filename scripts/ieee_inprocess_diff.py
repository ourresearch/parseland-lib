"""In-process IEEE diff: IEEE.parse() vs human-goldie.

Mirror of ``elsevier_inprocess_diff.py`` for the IEEE parser. Routes through
``IEEE.parse()`` directly — no HTTP, no dispatcher — so the parser delta is
unambiguous and not blended with generic-fallback behavior.

Per row:
  1. Resolve the latest harvest UUID via Taxicab.
  2. Read HTML from R2.
  3. Wrap in BeautifulSoup.
  4. Call ``IEEE(soup).parse()`` directly.
  5. Score against gold using parseland-eval's scorers.
  6. Aggregate.

Run:
    cd parseland-lib
    TAXICAB_URL=http://localhost:8081 \\
      ITER_ARTIFACT=tests/fixtures/ieee-iter1-before.json \\
      .venv/bin/python scripts/ieee_inprocess_diff.py
"""
...
ARTIFACT = Path(
    os.environ.get(
        "ITER_ARTIFACT",
        "/Users/shubh-trips/Documents/OpenAlex/parseland-lib/tests/fixtures/ieee-iter1-before.json",
    )
)
...
def resolve_latest_harvest_uuid(doi: str) -> str | None:
    ...
    IEEE landing pages live on ieeexplore.ieee.org. PDF or asset CDN URLs
    (e.g. ``ieeexplore.ieee.org/iel*``) sometimes appear as redirect-target
    records whose stored HTML is a binary stub. Prefer article landing-page
    records over asset CDN records, with most-recent-by-created_date as the
    tiebreaker.
    """
    try:
    ...
    # Asset / binary hosts — never first choice.
    ASSET_HOSTS = (
        "ieeexplore.ieee.org/ielx",  # PDF/binary CDN path
        "ieeexplore.ieee.org/iel",   # earlier binary CDN
    )
...
def parse_in_process(html: str) -> dict:
    """Run IEEE.parse() directly on the HTML. No HTTP, no dispatcher.
    ...
    ``parse_publisher_fulltext_location`` (what the production ``parse_page``
    pipeline runs). ``IEEE.parse()`` alone only handles authors + abstract;
    without this call the in-process eval would report zero PDFs even when
    production extracts them.
    """
...
def fmt_score_line(doi: str, has_bot_check: bool, score: dict, elapsed: float,
    ...
    ab = score["abstract"]
    pdf = score["pdf_url"]
    found_mark = "Y" if authors_found else "N"
    a_str = f"{a:.2f}" if a is not None else "  - "
    af_str = f"{af:.2f}" if af is not None else "  - "
    ab_str = "1.00 (M)" if ab.get("match_at_threshold") else f"{ab.get('fuzzy_ratio', 0):.2f}    "
    pdf_str = "M" if pdf.get("strict_match") else "."
    return (
        f"  found={found_mark}  {doi:<45} {bot}  "
...
def main():
    ...
    print(f"loaded {len(rows)} IEEE gold rows from {GOLD_NDJSON.name}\n")
    ...
    print(f"\n========== TL;DR AGGREGATE (n={n}) ==========")
    print(f"  IEEE.authors_found(): {n_authors_found}/{n} rows")
    ...
    artifact = {
        ...
        "measurement_method": "in-process IEEE.parse() — no HTTP, no dispatcher",
