"""In-process Springer diff: Springer.parse() vs human-goldie.

Mirror of scripts/elsevier_inprocess_diff.py for the SpringerLink publisher
parser. Same envelope, same scorers, same artifact shape — only the parser
class and default fixture paths change so iter before/after artifacts stay
parallel between publishers.

Env vars:
    TAXICAB_URL          override Taxicab base (default from parseland_eval.api)
    GOLD_NDJSON          path to gold NDJSON (default: tests/fixtures/springer-gold.ndjson)
    ITER_ARTIFACT        path to write the per-iter result (default: tests/fixtures/springer-iter1-before.json)
    PARSELAND_NO_PROGRESS=1  disable ProgressLogger writes

Run:

    cd parseland-lib
    GOLD_NDJSON=tests/fixtures/springer-gold.ndjson \\
      ITER_ARTIFACT=tests/fixtures/springer-iter1-before.json \\
      .venv/bin/python scripts/springer_inprocess_diff.py
"""
...
from parseland_lib.publisher.parsers.springer import Springer  # noqa: E402
from parseland_lib.s3 import get_landing_page_from_r2  # noqa: E402

GOLD_NDJSON = Path(
    os.environ.get(
        "GOLD_NDJSON",
        "/Users/shubh-trips/Documents/OpenAlex/parseland-lib/tests/fixtures/springer-gold.ndjson",
    )
)
ARTIFACT = Path(
    os.environ.get(
        "ITER_ARTIFACT",
        "/Users/shubh-trips/Documents/OpenAlex/parseland-lib/tests/fixtures/springer-iter1-before.json",
    )
)


def resolve_latest_harvest_uuid(doi: str) -> str | None:
    """Pick best harvest UUID for ``doi``. Prefer link.springer.com /
    springeropen.com / nature.com / biomedcentral.com hosts over PDF asset
    or generic redirector URLs."""
    try:
...
def parse_in_process(html: str) -> dict:
    """Run Springer.parse() directly on the HTML. No HTTP, no dispatcher.
...
def main():
    ...
    print(f"loaded {len(rows)} Springer gold rows from {GOLD_NDJSON.name}\n")
    ...
    progress = ProgressLogger(job_id=job_id, total=len(rows),
                              label=f"Springer inprocess diff → {ARTIFACT.name}") if use_progress else None
    ...
    print(f"\n========== TL;DR AGGREGATE (n={n}) ==========")
    print(f"  Springer.is_publisher_specific_parser(): {n_is_publisher}/{n} rows")
    print(f"  Springer.authors_found(): {n_authors_found}/{n} rows")
    if n_parse_errors:
    ...
    artifact = {
        ...
        "measurement_method": "in-process Springer.parse() — no HTTP, no dispatcher",
